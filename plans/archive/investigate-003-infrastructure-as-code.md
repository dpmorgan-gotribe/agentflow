...
- 'The question of WHETHER to investigate IaC/multi-env/CI-CD was worth asking; the answer was reachable without 60 min of formal investigation because MVP exit criterion is clear (PR awaiting human approval) and most of the concern clearly falls outside that scope.'
- 'Explicit non-targets in docs/build-tier-roadmap.md preserve scope discipline better than deferred-but-unscoped concerns. Named non-targets force the deferral to be visible to future readers.'
  abandoned-reason: 'Punted to post-mvp-scaffolding. MVP exit criterion (PR awaiting human approval; runs locally via docker-compose) does not require cloud deploy automation, multi-env separation, or IaC tooling. 3 new stubs added to post-mvp-scaffolding/: iac-stack-shelf.md, multi-env-deploy.md, ci-cd-deploy-automation.md. Roadmap patched with explicit non-targets.'
  test-results:
  summary: 'n/a (investigation never executed — punted before starting)'
  duration-minutes: 30
