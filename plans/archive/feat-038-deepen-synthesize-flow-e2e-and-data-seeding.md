...
## Completion record (2026-04-30)

All 6 phases shipped clean across one session. Empirical validation
on `repo-health-dashboard-01` confirmed the headline acceptance
criterion. Feature complete; book-swap is the next concrete consumer
when its E2E layer lands (Strategy C real-DB path).

### Phases shipped

| Phase     | Commit              | Scope                                                                                                 |
| --------- | ------------------- | ----------------------------------------------------------------------------------------------------- |
| 0         | `745bf78`           | testing-policy.md §E2E data-seeding strategy decision (per-stack-skill, not global)                   |
| 1         | `77b9024`           | v2.0 user-flows-manifest schema (Zod + JSON + 24 tests; backward-compat with all 5 shipped manifests) |
| 2A        | `ee8b88a`           | synthesizer interactions[] translator (10 InteractionStep kinds → Playwright statements)              |
| 2B        | (Phase 2B)          | per-strategy seed-helpers (4 templates) + persistence_layer field + architect skill §7d               |
| 3         | `ba04804`           | /user-flows-generator authors interactions[] + seedingTier (selector inference algorithm)             |
| Empirical | `3a8032b` (project) | flow-1 manually authored on `repo-health-dashboard-01`; spec failed at interaction 4 cleanly          |
| 4         | `3672090`           | parseFailureMessage v2.0-aware + new seed-setup primaryCause + classifier ladder                      |
| 5         | (this commit)       | fixture-driven validation harness (3 strategies × 6 tests; 568→576 passing)                           |

### Cumulative test deltas

- `@repo/orchestrator-contracts`: 374 → 398 (+24)
- `orchestrator`: 568 → 576 (+8 — runner v2.0 + seed-setup + 6 fixture-driven)

### Empirical validation result

Re-running `synthesize-flow-e2e.mjs` against `repo-health-dashboard-01`'s
manually-authored flow-1 produced a deterministic 6-statement spec.
With the backend NOT booted (the bug-032-reverted equivalent), the spec
failed at interaction 4 with:
...
The legacy v1.0 spec would have passed silently because `page.goto("/")`
succeeds even without a backend. This proves the kanban-09 / repo-health-01
class integration bug is now caught cleanly.

## Lessons learned

1. **Schema versioning was load-bearing.** The decision in Phase 1 to add
   `schemaVersion: "2.0"` (with optional new fields and v1.0 fallback in
   readers) meant Phases 2A → 5 could each ship additively without
   breaking the 5 in-flight projects. The 30-manual-cp-tax this session
   would have been 30 file-coordinated migrations otherwise.

2. **Inference fallback for missing fields paid for itself in Phase 3
   empirical validation.** `repo-health-dashboard-01`'s
   `architecture.yaml` predates the `persistence_layer` field; the
   synthesizer's inference (database null + backend non-null →
   external-api-only → Strategy D) correctly resolved without any
   architect re-run. New fields shipping in production should always
   carry a non-trivial inference path for legacy artefacts.

3. **The post-write formatter's invisible reformatting cost a Phase 5
   afternoon.** The fixture-driven test originally aimed for byte-equality
   compare against `expected/` snapshots — defeated by prettier rewriting
   quote styles + paren elision + line wrapping on every Write. Settled
   on structural-feature assertions instead (which catch every
   load-bearing semantic regression). The lesson: snapshot tests against
   formatter-touched files require either passing both sides through the
   same formatter OR retreating to structural assertions. Pre-baking the
   formatter into the synthesizer itself (option not pursued in Phase 5)
   would have made byte-equality reliable.

4. **Strategy is per-stack-skill, not global.** Phase 0's empirical
   survey across the factory's shipped projects revealed the Phase-0-spec's
   3-option framing (A/B/C global pick) was wrong — different projects
   need different strategies based on persistence_layer. The plan's
   §Phase 0 §Decision section captures the rationale; this is now the
   canonical seeding contract documented in
   `.claude/rules/testing-policy.md`.

5. **Prettier-formatted committed snapshots ARE useful for human
   reference even when they aren't load-bearing in tests.** The Phase 5
   `expected/flow-1.spec.ts` files remain committed as readable
   references even though byte-equality compare was abandoned. Future
   contributors inspecting the synthesizer's behavior can read those
   files instead of running the synthesizer themselves.

6. **The gitignore + worktree-aware sync (refactor-008) closed the right
   gap at the right moment.** Without that refactor, Phase 5's fixture
   trees + Phase 4's schema sync would each have triggered another round
   of manual cp invocations. Bundling the sync extension mid-session
   (rather than treating it as scope creep) saved measurable time on
   Phases 4-5.
