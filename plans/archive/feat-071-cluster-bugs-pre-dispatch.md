...
created: 2026-05-08
updated: 2026-05-13
outcome: shipped — Phase A (pure function + schema) + Phase B (loop wiring + propagation) both landed in one session; cluster-pass opt-in via clusterThreshold context option
...
### 2026-05-13 — Phase B shipped same session

Phase B landed in the same session as Phase A after user directed to push forward. Implementation:

- `FixBugsLoopContext.clusterThreshold?: number` (default undefined = OFF). Operator opt-in via env `FIX_BUGS_CLUSTER_THRESHOLD=N` is the documented activation pattern; cli-runner wiring deferred to first empirical use.
- `applyClusterPass(doc, threshold, bugsYamlPath)` helper: runs at iteration top. Filters candidates to `status === "pending" && clusterParent === null && clusterMembers === null && !errorLog.includes("[cluster-fallback]")`. Calls `clusterBugs()` from Phase A. On synth, mutates members with `clusterParent` + appends parent to `doc.bugs[]`. Writes bugs.yaml ONCE if mutated.
- Dispatch filter extended with `b.clusterParent === null` so tagged members skip the dispatch wave while their parent runs.
- `propagateClusterResolutions(doc, iteration, bugsYamlPath)` helper: runs at iteration BOTTOM after verify + flapping detection. For each cluster parent:
  - `parent.status === "completed"` → walk members with `clusterParent === parent.id`, flip to `completed` with `resolvedInIteration` set
  - `parent.status === "failed"` → walk members, clear `clusterParent`, append `[cluster-fallback]` errorLog marker (which the cluster-pass excludes-from-re-clustering filter reads)

Tests (`orchestrator/tests/fix-bugs-loop.test.ts`) — 5 new (1 extra beyond the planned 4 — dispatch-filter coverage):

1. "synthesizes a cluster parent + tags members at iteration top when threshold met"
2. "dispatch filter skips bugs with clusterParent set — only parent dispatches"
3. "parent completion → members flip to completed on next iteration"
4. "parent failure → members revert clusterParent:null (dispatch individually next iter)"
5. "below threshold → no cluster synthesized; bugs dispatch individually"

Cycle-prevention learning: the propagation's clearing of `clusterParent` triggered an unintended re-cluster on the next iteration's `applyClusterPass`. The cleared-clusterParent members were valid clustering candidates again → infinite loop of synth-fail-clear-resynth. Fix: `applyClusterPass` excludes bugs whose errorLog contains the `[cluster-fallback]` marker. Documented inline in the helper's comment.

Test-id learning: dispatch task ids carry the agent suffix shape `<bug-id>-<agent>` (e.g. `bug-parity-clustered-home-pixel-systemic-divergence-systemic-fixer`). Test assertions on `t.id` must match the suffixed shape or strip it.

Suite: 963/963 (was 958 Phase A + 5 Phase B). Plan moves to `status: completed` / outcome: shipped.
