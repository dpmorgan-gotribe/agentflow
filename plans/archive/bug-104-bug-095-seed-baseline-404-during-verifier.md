...
created: 2026-05-13
updated: 2026-05-14
outcome: shipped — env-spread order fixed (test-seed contract keys now AFTER process.env) + pre-flight health diagnostic added to bug-095 path for easier future debugging
...
### 2026-05-14 — env-spread order fix shipped

Diagnosis confirmed: `orchestrator/src/dev-server.ts spawnBackendDevServer` had
...
const backendEnv = {
  ...
  DATABASE_PATH: "...",
  LOG_LEVEL: "warn",
  ...process.env, // ← clobbers above defaults
  PORT: String(port),
};
```

When the operator's shell env carries `ENABLE_TEST_SEED` with a different value (or empty string), `...process.env` overrides the orchestrator's `=1` intent — the spawned Fastify process doesn't register `/test/*` routes, and bug-095's POST returns 404.

Fix: reorder so the test-seed contract keys + PORT come AFTER `process.env`:

```ts
const backendEnv = {
  DATABASE_PATH: "...", // operator-overridable
  LOG_LEVEL: "warn", // operator-overridable
  ...process.env,
  ENABLE_TEST_SEED: "1", // pinned — verifier contract
  PORT: String(port), // pinned — caller specifies port
};
```

Extracted as a pure helper `buildBackendSpawnEnv(parentEnv, port)` so tests don't need to mock spawn().

Also added bug-095 path diagnostic in `build-to-spec-verify.ts`: pre-flight GET /health BEFORE the seed-baseline POST, then label the warning depending on whether health was 200/404/connection-refused. Three distinct warning shapes now surface so future debugging can distinguish "API down" from "routes not registered" from "transient port collision".

Tests: 6 new in `tests/dev-server.test.ts` cover ENABLE_TEST_SEED preservation against parent-env `=0`, empty string, missing, PORT-override conflict, operator-DATABASE_PATH/LOG_LEVEL preserve, default-when-parent-empty.

Suite: 969/969 (was 963 + 6 new bug-104 env tests).

Empirical re-validation deferred: the next /build-to-spec-verify run on reading-log-02 should produce either "bug-095: restored seed-baseline (204)" (success) OR clear diagnostic labels if some other issue surfaces.
