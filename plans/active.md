...
| feat-030-quota-observability                | feature | draft  | P1       | feat/quota-observability                | `/quota-status` skill + 1-token SDK probe printing all 5 rateLimitType buckets with utilization% + persistent rate-limit-events.ndjson + warning-vs-rejection gate split + per-model cost breakdown in counters.json. Implements investigate-010 short-term recommendation.      |
| feat-031-prompt-cache-systemprompt          | feature | draft  | P2       | feat/prompt-cache-systemprompt          | Wire `Options.systemPrompt` array with `SYSTEM_PROMPT_DYNAMIC_BOUNDARY` + `excludeDynamicSections: true` so Mode B agent dispatches share a cacheable prefix. Estimated 30-50% input-token cut on dispatches 2-N. Implements investigate-010 medium-term recommendation.         |
