...
| bug-018-pm-skips-affects-files              | bug     | draft  | P1       | fix/pm-skips-affects-files              | PM agents silently skip populating `affects_files[]` in tasks.yaml + emit confabulated reasons ("not in schema" — false). 3/4 PMs failed on 2026-04-28 dispatch. Drops bug-015 Phase 2 file-affinity gate. Likely fix: tighten pm SKILL.md mandatory-fields block + add post-emit ≥80% coverage gate + force PM to read project schema before authoring. |
