...
| bug-024-tester-modifies-source                | bug     | draft  | P1       | bug/tester-modifies-source                | Tester agent modifies source files instead of flagging via genuineProductBugs[] — caused feat-error-states to hit 20-min wall-clock abort 2x in a row + permanent feature failure on repo-health-dashboard-01 run. Fix: hard-constraint section in tester.md + testing-policy.md upgrade to prescriptive + skill mirror.                                                                      |
| bug-025-cross-feature-url-contract            | bug     | draft  | P1       | bug/cross-feature-url-contract            | feat-home navigates to `/r/${owner}/${repo}` but feat-report's route is `/report/[owner]/[repo]/page.tsx` → 404 on form submit. Two builders disagreed on URL convention with no authoritative contract. Fix: add `route_pattern` field to screens.schema.json + stack-skill §Routing Contract + builder prompt to read screens.json before writing nav code.                                 |
| bug-026-api-client-import-extensions          | bug     | draft  | P1       | bug/api-client-import-extensions          | packages/api-client/src/index.ts uses Node-ESM `.js` import extensions → Webpack can't resolve → dev-server-compile error on web app. Backend-builder authored Node-style without checking only consumer is Webpack. Fix: drop `.js` (works in both Node + Webpack) + stack-skill §Cross-tier package conventions section.                                                                    |
| bug-028-audit-reachability-misses-router-push | bug     | draft  | P1       | bug/audit-reachability-misses-router-push | audit-app-reachability.mjs extracts nav targets from `<Link href>` JSX only — doesn't walk function-call ASTs for router.push / router.replace / redirect / permanentRedirect / next/form Form action. Dynamic routes navigated programmatically (form submits) get flap-looped forever. Fix: AST visitor extension + template-string matching against dynamic-segment routes.                |
