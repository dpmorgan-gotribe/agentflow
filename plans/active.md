...
| bug-021-checkout-feature-no-worktree-reuse       | bug     | draft  | P0       | fix/checkout-feature-no-worktree-reuse       | Discovered while attempting bug-020 recovery on repo-health-dashboard-01. Orchestrator's `runCheckoutFeature` (invoke-agent.ts:368) hard-fails with `stale-worktree` when the worktree dir already exists — there's NO code path to reuse an existing worktree on resume. The /resume-build SKILL.md design assumed the orchestrator would honor in-flight features and skip checkout-feature; the matching code was never implemented. Net effect: every `--resume-feature-graph --pipeline-run-id` attempt cascade-fails all in-flight features. Fix: feature-graph.ts skips checkout-feature for features in `progress.inFlight[]` + optional `reuseExisting` flag on the op for defense-in-depth. |
