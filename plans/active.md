...
| investigate-011-mode-b-wall-clock-reduction | investigation | draft  | P1       | -                                       | Time-axis analog of feat-031's 3-5× cost win. Where does Mode B wall-clock actually go, and which 3 levers deliver the highest savings? Hypothesized: per-task parallelism within builder, sequential agent_sequence (security+tester+reviewer all read builder output), Sonnet on mechanical tasks, per-worktree pnpm install, cache-warmup probe. |
