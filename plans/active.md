...
| bug-032-api-base-url-not-coordinated-with-backend-port | bug     | draft  | P0       | fix/api-base-url-not-coordinated-with-backend-port | Frontend `/api/*` requests 404 in dev — `NEXT_PUBLIC_API_BASE` empty by default → relative URL hits Next.js `:3000` instead of FastAPI `:8000`. User's constraint: dev orchestration must port-discover (don't hard-code). Phases: (A) author env contract per project; (B) `scripts/dev.mjs` boots backend, captures port, writes `NEXT_PUBLIC_API_BASE`, boots frontend; (C) factory-level wiring in architect skill + dev-server.ts auto-boot; (D) synthesize-flow-e2e baseURL signal. |
