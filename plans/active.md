...
| feat-044-shepherd-kanban-webapp-09-to-95         | feature       | abandoned | P0       | feat/quota-observability                  | ABANDONED 2026-04-30. kanban-09 has 100+ uncommitted source modifications + 18 broken unit tests (mid-rework). Operator pivoted; kanban-pre-build /start-build (step 11) produces a clean kanban instead. Phase A+B carryovers preserved. Superseded by feat-045.                                                 |
| feat-039-mock-interaction-step-kind              | feature       | completed | P0       | feat/quota-observability                  | F1 of investigate-012. SHIPPED 2026-04-30. Added `kind: "mock"` to InteractionStep union (zod + JSON schema); extended synthesizer to emit page.route() with method check + fulfill; new strategy-d-with-mock fixture; ordering test (mock BEFORE navigate); 404 contracts + 578 orchestrator tests passing.      |
| feat-040-live-backend-playwright-webserver       | feature       | completed | P1       | feat/quota-observability                  | SHIPPED 2026-04-30. react-next + svelte-kit SKILL.md §Testing now emit `webServer.command="node ../../scripts/dev.mjs"` for multi-tier (Strategy C/D); `pnpm exec next dev` for Strategy A. Retries bumped 0→1 for live-backend specs. Synced to all 12 react-next consumers.                                     |
| feat-041-node-trpc-nest-strategy-c-testing-block | feature       | completed | P1       | feat/quota-observability                  | SHIPPED 2026-04-30. Added Strategy C declaration + `/test/seed` + `/test/cleanup` Nest controller contract + ENABLE_TEST_SEED env gate to node-trpc-nest §3 (mirroring python-fastapi §3). Synced to book-swap-pre-build + book-swap. Unblocks book-swap Mode B.                                                  |
| feat-042-node-fastify-stack-skill                | feature       | completed | P0       | feat/quota-observability                  | SHIPPED 2026-04-30. Authored `.claude/skills/agents/back-end/node-fastify/SKILL.md` from scratch — 8 sections (layout, idioms, testing+Strategy C, commands, gotchas, review, deps, anti-patterns, self-verify). Synced to finance-track-pre-build + finance-track. Unblocks finance-track Mode B.                |
...
| feat-045-shepherd-repo-health-dashboard-01-to-95 | feature       | completed | P0       | feat/quota-observability                  | SHIPPED 2026-04-30 at 90/100 (manual-sanity 0/10 deferred). All measurable dims full marks; Flow E2E 8/8 pass in 13.3s. Surfaced + closed 2 factory-wide bugs (bug-033 + testing-policy hardening). First validation target proven; rubric works.                                                                 |
| bug-033-dev-mjs-env-not-propagated-to-fastapi    | bug           | completed | P0       | fix/dev-mjs-env-not-propagated-to-fastapi | SHIPPED 2026-04-30. scripts/dev.mjs now loads .env + .env.local into the subprocess env via parseEnvFile + loadEnvFiles. GITHUB_TOKEN reaches FastAPI; live /api/report/facebook/react returns 200 with rate_limit.remaining=4998 (authenticated 5000/hr). Synced to all 12 projects. Testing-policy.md hardened. |
