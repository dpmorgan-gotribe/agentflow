...
6. `architecture.yaml.tooling.icon_library` → which icon library the kit standardizes on. **This inverts the usual "user > research > defaults" precedence on purpose.** `assets.md` may suggest per-style icon libraries (Lucide for minimal, Phosphor for playful), but the kit ships with exactly one library for visual coherence across all apps, regardless of which style wins. The architect's project-level choice wins. (User-supplied icons in `asset-inventory.json.icons[]` still take precedence — they're used verbatim rather than swapped for library equivalents.)
