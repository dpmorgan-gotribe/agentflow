...
```json
{
  "hooks": {
    "PreToolUse": [
      ...
      {
        "matcher": "Write|Edit",
        "hooks": [{ "type": "command", "command": "bash .claude/hooks/enforce-boundaries.sh" }]
      },
      {
        "matcher": "*",
        "hooks": [{ "type": "command", "command": "node .claude/hooks/detect-loop.mjs" }]
      }
    ],
    "PostToolUse": [
      {
    ...
    "SessionStart": [
      {
        "hooks": [{ "type": "command", "command": "echo '{\"additionalContext\": \"If resuming work, run /load-context-chain to reconstruct state.\"}'" }]
      }
    ]
  }
}
